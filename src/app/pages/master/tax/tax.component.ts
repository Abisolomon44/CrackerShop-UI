...
export class TaxComponent {
  ...
  loadTaxes() {
    this.masterService.getTaxes().subscribe({
      ...
      error: () => this.swall.error('Error', 'Failed to load taxes!', () => this.focusTax())
    });
  }
  ...
  checkDuplicate() {
    this.duplicateError = this.taxes
      ...
      .some(t => t.taxName.trim().toLowerCase() === this.tax.taxName.trim().toLowerCase());
  }
  ...
  private validateTax(): boolean {
    ...
    this.checkDuplicate();
    if (!this.tax.taxName) {
      this.swall.warning('Validation', 'Tax Name is required!', () => this.focusTax());
      return false;
    }
    if (this.duplicateError) {
      this.swall.warning('Validation', 'Tax already exists!', () => this.focusTax());
      return false;
    }
    return true;
  }
    ...
    this.masterService.saveTax(this.tax).subscribe({
      next: (res: ApiResponse) => {
        if (res.success) {
          this.loadTaxes();
          this.resetTax();
          this.swall.success('Success', res.message || 'Tax saved successfully!', () => this.focusTax());
        } else {
          this.swall.error('Error', res.message || 'Something went wrong!', () => this.focusTax());
        }
      },
      error: () => this.swall.error('Error', 'Failed to save tax!', () => this.focusTax())
    });
  }
  ...
  deleteTax(t: Tax) {
    this.swall.confirm(`Delete ${t.taxName}?`, 'This will mark the tax as inactive.').then(result => {
      if (!result.isConfirmed) return;

      const userId = this.commonService.getCurrentUserId();
      const now = new Date().toISOString();

      const deleted: Tax = { ...t, isActive: false, updatedByUserID: userId, updatedAt: now, updatedSystemName: 'AngularApp' };

      this.masterService.saveTax(deleted).subscribe({
        next: (res: ApiResponse) => res.success ?
          (this.loadTaxes(), this.swall.success('Deleted!', res.message || 'Tax deleted!', () => this.focusTax())) :
          this.swall.error('Error', res.message || 'Failed to delete tax!', () => this.focusTax()),
        error: () => this.swall.error('Error', 'Failed to delete tax!', () => this.focusTax())
      });
    });
  }
